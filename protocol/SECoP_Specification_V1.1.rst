...
.. table:: basic messages (implementation is mandatory, even if functionality seems not to be needed.)
    ...
    ======================= ============== ==================
     `identification`_       request        ``*IDN?``
          \                  reply          ``ISSE&SINE2020,SECoP,date,version``
...
.. code::
  ...
  > *IDN?
  < ISSE&SINE2020,SECoP,V2019-09-16,v1.1
...
~~~~~~~~~~~~~~~~~~~~~~
``"optional"``:
    The names of optional struct elements.
...
- Added ``implementation`` property of modules
- Added ``controlled_by``, ``control_active`` and ``control_off``
- Added Features and the ``features`` module property
- Added ``target_limits``
- Added the ``HasOffset`` feature and the ``offset`` parameter
